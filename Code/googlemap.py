...
query = "gym near Maricopa County, AZ"
key = ""
BASE_DIR = "artifacts"
...
with open(
    ID_CSV_PATH, mode="w", newline="", encoding="utf-8", errors="ignore"
) as f:
...
for e in df["place_id"]:
    ...
    dd = pd.concat([dd, pd.DataFrame([dd_temp])], ignore_index=True)

Detail_CSV_PATH = os.path.join(BASE_DIR, "store", "result_detail.csv")
with open(
    Detail_CSV_PATH, mode="w", newline="", encoding="utf-8", errors="ignore"
) as f:
